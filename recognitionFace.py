...
id = 0
name = ['Tidak Diketahui', 'Dimas', 'Via','404']
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        id, confidance = faceRecognizer.predict(abuAbu[y: y+h, x:x+w])

        if confidance <= 50:
            nameID = name[id]
            confidanceTxt = " {}%".format(round(100-confidance))
        else:
            nameID = name[0]
            confidanceTxt = " {}%".format(round(100 - confidance))
    ...
    cv2.imshow('Recognition Wajah', frame)
